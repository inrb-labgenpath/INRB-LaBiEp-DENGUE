#! usr/bin/bash

#cd fastq &&
#for file in *barcode*; do cat $file/*.fastq > `echo $file`.fastq | chmod +x `echo $file`.fastq;done &&
#mv *.fastq ../reads ; cd .. &&

#cd fastq && 
#for i in *barcode??; do artic guppyplex --directory $i --min-length 150 --max-length 1500 --prefix ../trimmed_reads/DENV --quality 10;done && 
#cd .. &&

#snakemake -j1 -c10 -s visualization_bf.smk;
#snakemake -j1 -c10 -s visualization_af.smk;

#cd trimmed_reads;
#for i in *.fastq; do minimap2 -a -x map-ont -t 10 ../index/GCA_000001405.15_GRCh38_no_alt_plus_hs38d1_analysis_set.fna.gz `echo ${i:0:32}`.fastq -o ../mapping/`echo ${i:0:32}`.bam; done &&
#cd .. &&

#cd mapping;
#for i in *bam;do samtools fastq -F 3588 `echo ${i:0:32}`.bam | gzip -c > ../human_reads/`echo ${i:0:32}`.fastq.gz;done &&
#cd .. &&

#cd mapping;
#for i in *bam;do samtools fastq -F 3584 `echo ${i:0:32}`.bam | gzip -c > ../non_human_reads/`echo ${i:0:32}`.fastq.gz;done &&
#cd .. &&

#cd non_human_reads;
#for i in *.fastq.gz; do minimap2 -a -x map-ont -t 10 ../reference_dengue/DENV.reference.fasta `echo ${i:0:32}`.fastq.gz -o ../mapping/`echo ${i:0:32}`_DENV.bam; done &&
#cd .. &&

#cd mapping;
#for i in *bam;do samtools fastq -F 3588 `echo ${i:0:32}`_DENV.bam | gzip -c > ../dengue_reads/`echo ${i:0:32}`.fastq.gz;done &&
#cd .. &&

#cd mapping;
#for i in *bam;do samtools fastq -F 3584 `echo ${i:0:32}`_DENV.bam | gzip -c > ../non_dengue_reads/`echo ${i:0:32}`.fastq.gz;done &&
#cd .. &&

#cd dengue_reads;
#for i in *.fastq.gz; do mkdir -p ../bams/`echo ${i:0:32}` && minimap2 -a -x map-ont -t 10 ../reference_dengue/DENV.reference.fasta $i | samtools view -bS -F 4 - | samtools sort -o ../bams/`echo ${i:0:32}`/`echo ${i:0:32}`.sorted.bam -;done &&
#cd .. &&

#cd bams;
#for i in *; do samtools index $i/$i.sorted.bam;done &&
#cd .. &&

#cd bams;
#for i in *; do align_trim --normalise 200 ../reference_dengue/DENV.scheme.bed --start --remove-incorrect-pairs --report $i/$i.alignreport.txt < $i/$i.sorted.bam 2> ./$i/$i.alignreport.er | samtools sort -T $i - -o $i/$i.trimmed.rg.sorted.bam; done &&
#cd .. &&

#cd bams;
#for i in *; do align_trim --normalise 200 ../reference_dengue/DENV.scheme.bed --remove-incorrect-pairs --report $i/$i.alignreport.txt < $i/$i.sorted.bam 2> $i/$i.alignreport.er | samtools sort -T $i - -o $i/$i.primertrimmed.rg.sorted.bam; done &&
#cd .. &&

#cd bams;
#for i in *; do samtools index $i/$i.trimmed.rg.sorted.bam | samtools index $i/$i.primertrimmed.rg.sorted.bam; done &&
#cd .. &&

#cd bams;
#for i in *; do artic_make_depth_mask --store-rg-depths ../reference_dengue/DENV.reference.fasta $i/$i.primertrimmed.rg.sorted.bam $i/$i.coverage_mask.txt; done &&
#cd .. &&

#cd bams;
#for i in *; do samtools mpileup -aa -A -Q 0 -d 0 $i/$i.trimmed.rg.sorted.bam | ivar consensus -p $i/$i.fa -m 10 -n N -t 0.5; done &&
#cd .. &&

#cd bams;
#for i in *barcode*/*.fa ; do cp -r $i ../consensus/; done &&
#cd ..

cd consensus;
bash ../coverage/coverage_fasta.sh &&
cd ..






